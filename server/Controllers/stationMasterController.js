...
async function insertIntoStationMaster(req, res) {
    ...
    try {
        ...
        if (selectResult.length > 0) {
            ...
            const [insertResult] = await db.promise().query(insertQuery, [stationName, productName.label, reportType, stationParameter, multipleMachines,process_number,cycleTime,dailyCount,productPerHour])
            const selectQuery = "SELECT station_id FROM station_master WHERE station_name = ? && product_name = ?"
            const [selectResult] = await db.promise().query(selectQuery, [stationName, productName.label])
            const station_id = selectResult[0].station_id;
...
async function updateStationMaster(req, res) {
    ...
    try {
        if (stationId === undefined) {
            ...
            const updateStationMasterQuery = "UPDATE station_master SET process_number=?,station_name = ?, product_name = ?, report = ?, station_parameters = ?, multiple_machine = ?,cycle_time=?,daily_count=?,product_per_hour=? WHERE station_id = ?"
            const [updateStationMasterResult] = await db.promise().query(updateStationMasterQuery, [process_number,stationName, productName, reportType, stationParameter, multipleMachines, cycleTime, dailyCount, productPerHour, stationId])
...
async function getStationNamesForOneProduct(req, res) {
    ...
    try {
        ...
        const searchQuery = "SELECT station_name,station_id,next_station_name,position FROM station_master WHERE product_name = ?"
