...
async function login(req, res) {
  console.log(req.body)
    ...
    try {
      const selectUserQuery = "SELECT password,employee_id,user_name,access_given FROM employee_master WHERE user_name = ?";
      ...
      if (selectUserResult.length === 0) {
        ...
        bcrypt.compare(password, hashedPassword, async (err, result) => {
          if (err) {
            ...
            res.status(201).send({ msg: "Done:Login successful",employeeId:selectUserResult[0].employee_id,userName:selectUserResult[0].user_name,access:selectUserResult[0].access_given,token:token });
            return;
          } else {
