...
async function getOneWorkerStation(req,res){
    ...
    try {
        const selectQuery = "SELECT DISTINCT sa.station_name, sm.position FROM station_allocation AS sa INNER JOIN station_master AS sm ON sa.station_name = sm.station_name WHERE employee_id=? AND toDate='0000-00-00' AND shift_id=?"
