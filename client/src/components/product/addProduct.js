    ...
    if (files.length) {
        ...
        reader.onload = (event) => {
            ...
            if (sheets.length) {
                ...
                rows.forEach((row)=>{
                  console.log(row.parameterName);
                  formik.values.parameters.push({ parameterName: row.parameterName, minVal: row.minVal, maxVal: row.maxVal, unit: row.unit,evaluation:row.evaluation,sample_size:row.sample_size,unitPresent:row.unitPresent,parameterStatus:row.parameterStatus })
                  }
                )
